...
const countryInfoCache = createAsyncTtlCache<CountryInfoRouteCacheValue>(COUNTRY_INFO_CACHE_TTL_MS, 256);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const cachedValue = await countryInfoCache.get(countryCode, async () => {
      ...
      const data: CountryInfoResponse = JSON.parse(text);
      ...
      return { countryCode, destinations, rawDestinations };
    });
