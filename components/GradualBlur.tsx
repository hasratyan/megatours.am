...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const mobileValue = config[mobileKey] as string | undefined;
      const tabletValue = config[tabletKey] as string | undefined;
      const desktopValue = config[desktopKey] as string | undefined;

      if (w <= 480 && mobileValue) v = mobileValue;
      else if (w <= 768 && tabletValue) v = tabletValue;
      else if (w <= 1024 && desktopValue) v = desktopValue;
      setVal(v);
    };
...
type GradualBlurExport = React.MemoExoticComponent<React.FC<PropsWithChildren<GradualBlurProps>>> & {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
};

const GradualBlurMemo = React.memo(GradualBlur) as unknown as GradualBlurExport;
